...
allEqual [] = True
allEqual (x : xs) = xs == (fst $ partition (\check -> check == x) xs)
...
rangeOf input_list = maximum input_list - minimum input_list
...
longest lists =
    let 
        breakTie :: (Ord a) => [[a]] -> [a]
        breakTie lists = head $ filter (\xs ->  head xs == (minimum $ map head lists)) lists
    in    
        case length big_lists of 1 ->    head big_lists
                                 _ ->    breakTie big_lists
    where
        big_lists = [ xs | xs <- lists, length xs == (maximum $ map length lists) ]
...
maxIndex arr = findIndex (maximum $ elems arr) (assocs arr) 
    where
        findIndex :: (Eq b) => b -> [(a,b)] -> a
        findIndex el ( (index, b) : rest)  = if el == b then index else findIndex el rest