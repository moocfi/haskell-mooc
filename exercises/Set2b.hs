...
oddFactorial n = if n <= 1 then 1 else case mod n 2  of 1-> n * oddFactorial (n-2)
                                                        0 -> (n-1) * oddFactorial (n - 3)
...
myGcd a 0 = a
myGcd 0 b = b
myGcd a b = if a == b then a else (if a < b then myGcd a (b-a) else myGcd (a-b) b)
...
countdown n = "Ready! " ++ countdownNumbers n ++ "Liftoff!"
    where   countdownNumbers 0 = ""
            countdownNumbers n = show n ++ "... " ++ countdownNumbers (n-1) 
...
smallestDivisor n = checkForDivisor n 2
    where checkForDivisor n k = if mod n k == 0 then k else checkForDivisor n (k + 1)
